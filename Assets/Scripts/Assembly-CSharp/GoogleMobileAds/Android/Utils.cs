	...
	internal class Utils
	{
		...
		public static AndroidJavaObject GetAdRequestJavaObject(AdRequest request)
		{
			...
			foreach (string testDevice in request.TestDevices)
			{
				...
				{
					string text = new AndroidJavaClass("com.google.android.gms.ads.AdRequest").GetStatic<string>("DEVICE_ID_EMULATOR");
					androidJavaObject.Call<AndroidJavaObject>("addTestDevice", new object[1] { text });
				}
				else
				{
			...
			{
				androidJavaObject.Call<AndroidJavaObject>("tagForChildDirectedTreatment", new object[1] { request.TagForChildDirectedTreatment == true });
			}
			androidJavaObject.Call<AndroidJavaObject>("setRequestAgent", new object[1] { "unity-3.4.0" });
			...
			foreach (MediationExtras mediationExtra in request.MediationExtras)
			{
				...
				{
					androidJavaObject6.Call<string>("put", new object[2] { extra2.Key, extra2.Value });
				}
